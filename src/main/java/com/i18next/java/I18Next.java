...
public class I18Next {
  ...
  private final JSONObject mRootObject = new JSONObject();
  ...
  private String[] splitKeyPath(String key) {
    if (key != null) {
      return key.split(Pattern.quote(mOptions.getKeySeparator()));
    }
    return null;
  }
  ...
  private String getNamespace(String key) {
    if (key != null) {
      ...
      if (indexOfNS > 0) {
        ...
        LOG.warn("namespace found for key '{}': {}", key, namespace);
        return namespace;
      }
    }
  ...
  private void initDefaultNamespaceIfNeeded(String nameSpace) {
    if (mOptions.getDefaultNamespace() == null) {
      LOG.warn("namespace taken from the first key available (it's now our default namespace): {}",
          nameSpace);
    ...
    if (keys != null && keys.length > 0) {
      for (String key : keys) {
        ...
        if (innerProcessValue != null) {
          ...
          LOG.warn("impossible to found key '{}'", key);
        }
      }
    }
  ...
  private String getValueRaw(String key, Operation operation) {
    ...
    if (namespace != null) {
      ...
      if (value == null && operation instanceof Operation.PreOperation) {
        String repreProcessedKey = ((Operation.PreOperation) operation)
            .preProcessAfterNoValueFound(key);
  ...
  private String getValueRawWithoutPreprocessing(String namespace, String key) {
    ...
    if (splitKeys == null) {
      ...
      if (value == null) {
        value = getValueRawByLanguageWithNamespace(mOptions.getFallbackLanguage(), namespace,
            splitKeys);
      }
    }
    return value;
  ...
  private String getValueRawByLanguageWithNamespace(String lang, String namespace,
      String[] splitKeys) {
  ...
  private String getRawWithNestingReplaced(String raw, Operation operation) {
    ...
    if (raw != null && raw.length() > 0 && reusePrefix != null && reuseSuffix != null
        && reusePrefix.length() > 0 && reuseSuffix.length() > 0) {
